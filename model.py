...
class AutoEncoderRNN(nn.Module):
    def __init__(self, input_size, hidden_size, output_size, num_layers=2, seq_len = 10, bidirectional=False, batch_size = 20, batch_first = True):
        super(AutoEncoderRNN, self).__init__()
        self.sequence_length = seq_len
        self.encoding = torch.zeros((batch_size, seq_len, hidden_size))
        self.dense1 = nn.Linear(input_size, input_size) 
        self.encoder = EncoderRNN(input_size, hidden_size, num_layers, bidirectional, batch_first = batch_first)
        self.decoder = DecoderRNN(hidden_size, input_size, num_layers, bidirectional, batch_first = batch_first)
        self.dense2 = nn.Linear(input_size, input_size)
        self.predictor = DecoderRNN(hidden_size, output_size, num_layers, bidirectional=bidirectional, batch_first = batch_first)
        #self.dense4 = nn.Linear(output_size, output_size)
